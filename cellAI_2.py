...
navigation = st.sidebar.radio("", options=["Home", "Analyse et Résultats"])
    ...
    with text_outil:
        ...
        st.markdown("""
            <div style="
                ...
                border-radius: 5px;">
                ⚠️ <strong>IMPORTANT:</strong> Cet outil est conçu pour assister les professionnels de santé et ne remplace pas l'expertise médicale.
            </div>
            """, unsafe_allow_html=True)
...
# image méthodologie
    ...
    with text_met:
        ...
        st.markdown("""
            <div class="txt_met">
                <h3>Méthodologie</h3>
                <p>Cette application utilise un modèle d'intelligence artificielle entraîné sur des milliers d'images</p>
                <p>histopathologiques annotées par des experts. L'algorithme analyse les caractéristiques morphologiques</p>
                <p>des tissus pour identifier les patterns associés aux différents types de néoplasies.</p>
            </div>
            """, unsafe_allow_html=True)

# image confidentialité
    text_conf, img_conf= st.columns([4,1])
    ...
    with text_conf:
        ...
        st.markdown("""
            <div class="text_conf">
                ...
                <p>Toutes les images téléchargées sont traitées de manière sécurisée et ne sont pas conservées au-delà de la session d analyse.</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if uploaded_file is not None:
                ...
                # Envoi de l'image à l'API
                    ...
                    if res.status_code == 200:
                        ...
                        st.markdown("")
                        st.markdown(f"""
                                <p style="margin-left: 20px; font-weight: bold; font-size: 14px;">
                                    Diagnostic principal: {result_data.get('diag')}
                                </p>
                                    """, unsafe_allow_html=True)
                        if res.headers.get('p_class_d') == 'tumor':
                            st.markdown("""
                                        <p style="margin-left: 20px; font-size: 14px;">
                                        Carcinome infiltrant de type non spécifique </p>
                                        <p style="margin-left: 20px; font-size: 14px;">
                                        Échantillon tumoral inclus en paraffine pour génétique somatique"</p>
                                        <p style="margin-left: 20px; font-size: 14px;">
                                        Absence de facteur confondant à type de nécrose, fibrose, ou mucine."</p>
                                    """, unsafe_allow_html=True)

                            st.markdown(f"""
                                <p style="margin-left: 20px; font-weight: bold; font-size: 14px;">
                                    Niveau de confiance pour le diagnostic: {'{:.2f}'.format(float(result_data.get('c_diag')) * 100)} %
                                </p>
                                    """, unsafe_allow_html=True)

                            st.markdown(f"""
                                <p style="margin-left: 20px; font-weight: bold; font-size: 14px;">
                                    Pourcentage de cellules tumorales dans la zone sélectionnée: {result_data.get('p_class_tx').upper()}
                                </p>
                                    """, unsafe_allow_html=True)

                            st.markdown(f"""
                                <p style="margin-left: 20px; font-weight: bold; font-size: 14px;">
                                    Niveau de confiance pour le Taux de Cellularité: {'{:.2f}'.format(float(result_data.get('c_tx')) * 100)} %
                                </p>
                                    """, unsafe_allow_html=True)
...
# Card 4 - Interprétation par Claude
    with col4:
        st.markdown("""
        <div style="border: 1px solid #ddd; margin-bottom: 20px; border-radius: 5px; background-color: #ffffff; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
            <div style="text-align: center; font-size: 20px; background-color: #f8f9fa; padding: 10px;font-weight: bold; border-bottom: 1px solid #ddd;">
